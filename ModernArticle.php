<!-- START:: INCLUDING THE HEADER -->
<?php include('layouts/modern_index/ModernIndexHeader.php')?>
<!-- END:: INCLUDING THE HEADER -->

<!-- START:: ARTICLE HEDAER IMAGE SECTION -->
<div class="article_header_img_wraper_modern" style="background: url('assets/media/pic5.jpeg');">
  <div class="overlay">
    <div class="intro_text_wraper">
      <h1> <span>A</span>rticle <span>T</span>itle  </h1>
    </div>
  </div>

  <div class="section_curve_wraper">
    <svg data-name="Layer 1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1200 120" preserveAspectRatio="none">
      <path d="M741,116.23C291,117.43,0,27.57,0,6V120H1200V6C1200,27.93,1186.4,119.83,741,116.23Z" class="shape-fill"></path>
    </svg>
  </div>
</div>
<!-- END:: ARTICLE HEDAER IMAGE SECTION -->

<!-- START:: SIDEBAR ARTICLE -->
<div class="article_modern"> 
  <div class="container overflow-hidden">
    <div class="row justify-content-center wow fadeIn" data-wow-duration="1.5s" data-wow-delay="0.5s">
      <div class="col-12 col-md-10">
        <div class="articl_body">
          <h5>
            <i class="far fa-calendar-alt"></i>
            <span> 1 - 5 - 2021 </span> 
          </h5>
          Lorem ipsum dolor, sit amet consectetur adipisicing elit. 
          Impedit maxime voluptate similique sunt molestias. Aspernatur 
          error non, vero deserunt cumque inventore expedita eveniet 
          odio quasi adipisci saepe, quia distinctio repellat.
          Lorem ipsum dolor, sit amet consectetur adipisicing elit. 
          Impedit maxime voluptate similique sunt molestias. Aspernatur 
          error non, vero deserunt cumque inventore expedita eveniet 
          odio quasi adipisci saepe, quia distinctio repellat.
          Lorem ipsum dolor, sit amet consectetur adipisicing elit. 
          Impedit maxime voluptate similique sunt molestias. Aspernatur 
          error non, vero deserunt cumque inventore expedita eveniet 
          odio quasi adipisci saepe, quia distinctio repellat.
          Lorem ipsum dolor, sit amet consectetur adipisicing elit. 
          Impedit maxime voluptate similique sunt molestias. Aspernatur 
          error non, vero deserunt cumque inventore expedita eveniet 
          odio quasi adipisci saepe, quia distinctio repellat.
          Lorem ipsum dolor, sit amet consectetur adipisicing elit. 
          Impedit maxime voluptate similique sunt molestias. Aspernatur 
          error non, vero deserunt cumque inventore expedita eveniet 
          odio quasi adipisci saepe, quia distinctio repellat.
          Lorem ipsum dolor, sit amet consectetur adipisicing elit. 
          Impedit maxime voluptate similique sunt molestias. Aspernatur 
          error non, vero deserunt cumque inventore expedita eveniet 
          odio quasi adipisci saepe, quia distinctio repellat.
          Lorem ipsum dolor, sit amet consectetur adipisicing elit. 
          Impedit maxime voluptate similique sunt molestias. Aspernatur 
          error non, vero deserunt cumque inventore expedita eveniet 
          odio quasi adipisci saepe, quia distinctio repellat.
          Lorem ipsum dolor, sit amet consectetur adipisicing elit. 
          Impedit maxime voluptate similique sunt molestias. Aspernatur 
          error non, vero deserunt cumque inventore expedita eveniet 
          odio quasi adipisci saepe, quia distinctio repellat.
          Lorem ipsum dolor, sit amet consectetur adipisicing elit. 
          Impedit maxime voluptate similique sunt molestias. Aspernatur 
          error non, vero deserunt cumque inventore expedita eveniet 
          odio quasi adipisci saepe, quia distinctio repellat.
          Lorem ipsum dolor, sit amet consectetur adipisicing elit. 
          Impedit maxime voluptate similique sunt molestias. Aspernatur 
          error non, vero deserunt cumque inventore expedita eveniet 
          odio quasi adipisci saepe, quia distinctio repellat.
          Lorem ipsum dolor, sit amet consectetur adipisicing elit. 
          Impedit maxime voluptate similique sunt molestias. Aspernatur 
          error non, vero deserunt cumque inventore expedita eveniet 
          odio quasi adipisci saepe, quia distinctio repellat.
          Lorem ipsum dolor, sit amet consectetur adipisicing elit. 
          Impedit maxime voluptate similique sunt molestias. Aspernatur 
          error non, vero deserunt cumque inventore expedita eveniet 
          odio quasi adipisci saepe, quia distinctio repellat.
        </div>
      </div>
    </div>

    <div class="row flex-column align-items-center justify-content-center mt-3 overflow-hidden">

      <div class="header_wraper my-4">
        <h2 class="wow bounceIn" data-wow-duration="1.5s" data-wow-delay="0.5s">
          <span></span>
          <span></span>
          Related Articles
        </h2>
      </div>

      <div id="articles" class="owl-carousel owl-theme col-12 wow bounceIn" data-wow-duration="1.5s" data-wow-delay="0.5s">

        <div class="item">
          <a href="ModernArticle.php">
            <div class="article_card_wraper">
              <div class="img_wraper">
                <div class="overlay">
                  <i class="fas fa-info-circle fa-3x"></i>
                </div>
                <img class="card_img" src="assets/media/pic1.jpeg" alt="">
                <div class="card_curve">
                  <svg data-name="Layer 1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1200 120" preserveAspectRatio="none">
                    <path d="M1200,0H0V120H281.94C572.9,116.24,602.45,3.86,602.45,3.86h0S632,116.24,923,120h277Z" class="shape-fill"></path>
                  </svg>
                </div>
              </div>
  
              <div class="card_title_wraper">
                <h4> Article Title 1 </h4>
                <h6> 1 - 5 - 2021 </h6>
              </div>
  
              <div class="card_text">
                Lorem ipsum dolor sit amet consectetur adipisicing elit. 
                Et corrupti minus ipsa aut, tenetur laboriosam nisi soluta 
                facilis. Quas ex provident quam, inventore consectetur 
              </div>
            </div>
          </a>
        </div>

        <div class="item">
          <a href="ModernArticle.php">
            <div class="article_card_wraper">
              <div class="img_wraper">
                <div class="overlay">
                  <i class="fas fa-info-circle fa-3x"></i>
                </div>
                <img class="card_img" src="assets/media/pic2.jpeg" alt="">
                <div class="card_curve">
                  <svg data-name="Layer 1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1200 120" preserveAspectRatio="none">
                    <path d="M1200,0H0V120H281.94C572.9,116.24,602.45,3.86,602.45,3.86h0S632,116.24,923,120h277Z" class="shape-fill"></path>
                  </svg>
                </div>
              </div>
  
              <div class="card_title_wraper">
                <h4> Article Title 2 </h4>
                <h6> 1 - 5 - 2021 </h6>
              </div>
  
              <div class="card_text">
                Lorem ipsum dolor sit amet consectetur adipisicing elit. 
                Et corrupti minus ipsa aut, tenetur laboriosam nisi soluta 
                facilis. Quas ex provident quam, inventore consectetur 
              </div>
            </div>
          </a>
        </div>

        <div class="item">
          <a href="ModernArticle.php">
            <div class="article_card_wraper">
              <div class="img_wraper">
                <div class="overlay">
                  <i class="fas fa-info-circle fa-3x"></i>
                </div>
                <img class="card_img" src="assets/media/pic3.jpeg" alt="">
                <div class="card_curve">
                  <svg data-name="Layer 1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1200 120" preserveAspectRatio="none">
                    <path d="M1200,0H0V120H281.94C572.9,116.24,602.45,3.86,602.45,3.86h0S632,116.24,923,120h277Z" class="shape-fill"></path>
                  </svg>
                </div>
              </div>
  
              <div class="card_title_wraper">
                <h4> Article Title 3 </h4>
                <h6> 1 - 5 - 2021 </h6>
              </div>
  
              <div class="card_text">
                Lorem ipsum dolor sit amet consectetur adipisicing elit. 
                Et corrupti minus ipsa aut, tenetur laboriosam nisi soluta 
                facilis. Quas ex provident quam, inventore consectetur 
              </div>
            </div>
          </a>
        </div>

        <div class="item">
          <a href="ModernArticle.php">
            <div class="article_card_wraper">
              <div class="img_wraper">
                <div class="overlay">
                  <i class="fas fa-info-circle fa-3x"></i>
                </div>
                <img class="card_img" src="assets/media/pic4.jpeg" alt="">
                <div class="card_curve">
                  <svg data-name="Layer 1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1200 120" preserveAspectRatio="none">
                    <path d="M1200,0H0V120H281.94C572.9,116.24,602.45,3.86,602.45,3.86h0S632,116.24,923,120h277Z" class="shape-fill"></path>
                  </svg>
                </div>
              </div>
  
              <div class="card_title_wraper">
                <h4> Article Title 4 </h4>
                <h6> 1 - 5 - 2021 </h6>
              </div>
  
              <div class="card_text">
                Lorem ipsum dolor sit amet consectetur adipisicing elit. 
                Et corrupti minus ipsa aut, tenetur laboriosam nisi soluta 
                facilis. Quas ex provident quam, inventore consectetur 
              </div>
            </div>
          </a>
        </div>

        <div class="item">
          <a href="ModernArticle.php">
            <div class="article_card_wraper">
              <div class="img_wraper">
                <div class="overlay">
                  <i class="fas fa-info-circle fa-3x"></i>
                </div>
                <img class="card_img" src="assets/media/pic5.jpeg" alt="">
                <div class="card_curve">
                  <svg data-name="Layer 1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1200 120" preserveAspectRatio="none">
                    <path d="M1200,0H0V120H281.94C572.9,116.24,602.45,3.86,602.45,3.86h0S632,116.24,923,120h277Z" class="shape-fill"></path>
                  </svg>
                </div>
              </div>
  
              <div class="card_title_wraper">
                <h4> Article Title 5 </h4>
                <h6> 1 - 5 - 2021 </h6>
              </div>
  
              <div class="card_text">
                Lorem ipsum dolor sit amet consectetur adipisicing elit. 
                Et corrupti minus ipsa aut, tenetur laboriosam nisi soluta 
                facilis. Quas ex provident quam, inventore consectetur 
              </div>
            </div>
          </a>
        </div>

        <div class="item">
          <a href="ModernArticle.php">
            <div class="article_card_wraper">
              <div class="img_wraper">
                <div class="overlay">
                  <i class="fas fa-info-circle fa-3x"></i>
                </div>
                <img class="card_img" src="assets/media/pic6.jpeg" alt="">
                <div class="card_curve">
                  <svg data-name="Layer 1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1200 120" preserveAspectRatio="none">
                    <path d="M1200,0H0V120H281.94C572.9,116.24,602.45,3.86,602.45,3.86h0S632,116.24,923,120h277Z" class="shape-fill"></path>
                  </svg>
                </div>
              </div>
  
              <div class="card_title_wraper">
                <h4> Article Title 6 </h4>
                <h6> 1 - 5 - 2021 </h6>
              </div>
  
              <div class="card_text">
                Lorem ipsum dolor sit amet consectetur adipisicing elit. 
                Et corrupti minus ipsa aut, tenetur laboriosam nisi soluta 
                facilis. Quas ex provident quam, inventore consectetur 
              </div>
            </div>
          </a>
        </div>

        <div class="item">
          <a href="ModernArticle.php">
            <div class="article_card_wraper">
              <div class="img_wraper">
                <div class="overlay">
                  <i class="fas fa-info-circle fa-3x"></i>
                </div>
                <img class="card_img" src="assets/media/pic7.jpg" alt="">
                <div class="card_curve">
                  <svg data-name="Layer 1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1200 120" preserveAspectRatio="none">
                    <path d="M1200,0H0V120H281.94C572.9,116.24,602.45,3.86,602.45,3.86h0S632,116.24,923,120h277Z" class="shape-fill"></path>
                  </svg>
                </div>
              </div>
  
              <div class="card_title_wraper">
                <h4> Article Title 7 </h4>
                <h6> 1 - 5 - 2021 </h6>
              </div>
  
              <div class="card_text">
                Lorem ipsum dolor sit amet consectetur adipisicing elit. 
                Et corrupti minus ipsa aut, tenetur laboriosam nisi soluta 
                facilis. Quas ex provident quam, inventore consectetur 
              </div>
            </div>
          </a>
        </div>
      </div>
    </div>
  </div>
</div>
<!-- END:: SIDEBAR ARTICLE -->

<!-- START:: INCLUDING THE FOOTER -->
<?php include('layouts/modern_index/ModernIndexFooter.php')?>
<!-- END:: INCLUDING THE FOOTER -->